...
int main(int argc, char *argv[])
{
  ...
  std::string odom_global_frame="";

  next_goal.param<std::string>("odom_name", odom_name, "/odom");
  next_goal.param<std::string>("odom_global_frame", odom_global_frame, "");


  ros::Subscriber sub1 = next_goal.subscribe(odom_name, 1000, pose_callback);
  ...
  while (ros::ok())
  {
    ...
    if (!planned_path.Path.empty())
    {
      ...
      {
        if(odom_global_frame == ""){
            goal_msgs.header.frame_id = "odom";

        }else{

            goal_msgs.header.frame_id = odom_global_frame;
