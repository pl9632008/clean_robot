...
CleaningPathPlanning::CleaningPathPlanning(costmap_2d::Costmap2DROS *costmap2d_ros)
{
    ...
    SIZE_OF_CELL = 3;
    if (private_nh.searchParam("size_of_cell", sizeOfCellString)) //��������,��������"size of cell"��������������Ӧ�����µĲ���ֵ����sizeOfCellString.
        private_nh.param("size_of_cell", SIZE_OF_CELL, 3);   //���û�����ռ��n*n��դ�񣬾����滮��ϡ��   
...
vector<geometry_msgs::PoseStamped> CleaningPathPlanning::GetPathInROS()
{
    ...
    if (!pathVecInROS_.empty())
        pathVecInROS_.clear(); //��ղ���
    geometry_msgs::PoseStamped posestamped;
    geometry_msgs::Pose pose;
    vector<cellIndex> cellvec;
    cellvec = GetPathInCV();
    /*trasnsform*/
    vector<cellIndex>::iterator iter; //cellIndex�����ŵ����У����Լ��Ƕ���Ϣ��
...
void CleaningPathPlanning::GetBorderTrackingPathInCV(vector<cv::Point2i> &resultVec)
{
    ...
    double dist, minDist;
    visitedVec.assign(borderPointsIndexVec.size(), false); //vector assign��vector�����µĸ�ֵ������
...
{
    cellMat = Mat(srcImg.rows / SIZE_OF_CELL, srcImg.cols / SIZE_OF_CELL, srcImg.type()); //cellMat����֮ǰ�涨��cellΪ��С��λ���»��ֵľ���
    ...
    for (i = 0; i < neuralizedMat.rows; i++)
    {
        for (j = 0; j < neuralizedMat.cols; j++)
        {
            if (cellMat.at<uchar>(i, j) == costmap_2d::LETHAL_OBSTACLE)
                ...
                neuralizedMat.at<float>(i, j) = 50.0 / j; //�����1.0/j��������ʲô��������û�п��ǵ�j=0ʱ�̵������أ�   hjrע
        }
    }
    return;
}
...
{
    int i = 0, j = 0;
    Point initpoint = Point(pathVec[0].col * SIZE_OF_CELL + SIZE_OF_CELL / 2, pathVec[0].row * SIZE_OF_CELL + SIZE_OF_CELL / 2); //������ȡ����λ�õ���˼��
    for (i = 1; i < pathVec.size(); i++)
    {
        ...
        {
            line(resultmat, initpoint, cupoint, Scalar(0, 255, 0), 1, 8); //line���ǻ��ߺ���
        }
        else
        ...
        */
        ...
        if (max_v <= 0) //������Ӧ�����ڴ��������ϵĹ�ϵ��
        {
            ...
            for (it = freeSpaceVec_.begin(); it != freeSpaceVec_.end(); it++)
            {
                ...
                {
                    if (Boundingjudge((*it).row, (*it).col)) //��Χ�Ƿ����-250�ĵ�
                    {
...
bool CleaningPathPlanning::initializeCoveredGrid() //�������Ҷ�CoverGrid������Ϊ����դ��
{
    boost::unique_lock<costmap_2d::Costmap2D::mutex_t> lock(*(costmap2d_->getMutex()));
    double resolution = costmap2d_->getResolution(); //�ֱ���

    covered_path_grid_.header.frame_id = "map"; //covered_path_grid_��costmap���е�ռ��դ���ͼ��Ϣ��
    ...
    double wx, wy;
    costmap2d_->mapToWorld(0, 0, wx, wy); //�ӵ�ͼ����ϵת������������ϵ��
    ...
    covered_path_grid_.data.resize(covered_path_grid_.info.width * covered_path_grid_.info.height); //�����������Ϊһ���ж��ٸ�դ�����Գ�����

    unsigned char *data = costmap2d_->getCharMap(); //����һ��ָ�룬ָ��һ���ײ��޷����ַ����飬�����д洢�Ŵ���ֵ���������ò�ƻ�����������Ϊ���۵�ͼ��
    for (unsigned int i = 0; i < covered_path_grid_.data.size(); i++)
    {
        ...
        covered_path_grid_.data[i] = data[i]; //����������Ϊ������ֵ���赽դ���ͼ��ÿ����Ӧդ����ȥ��
    }
    return true;
}
